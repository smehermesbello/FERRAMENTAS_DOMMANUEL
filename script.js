        ...
        lote.forEach(item => {
            page.innerHTML += `
                <div style="width:90mm; height:63mm; border:0.5pt solid #eee; display:flex; flex-direction:column; background:white;">
                    <div style="height:17.5mm; border-bottom:2pt dotted ${cor}; display:flex; align-items:center; padding:5px;">
...
function renderCarometro(data) {
    ...
    data.forEach(item => {
        ...
        page.innerHTML = `
            <div class="container-carometro" style="border-width:4pt; border-color:${cor};">
                <img src="${item.url}" class="foto-carometro">
            </div>
            <div style="font-family:'SFT-Round'; font-size:44pt; margin-top:20px; color:black;">${item.nome}</div>`;
        area.appendChild(page);
    });
    setupBtns(['pdf', 'ppt']);
}
...
async function doPDF() {
    ...
    // Garantir que todas as imagens (incluindo logo) estão carregadas
