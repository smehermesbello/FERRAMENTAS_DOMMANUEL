...
async function executarGeracao() {
    ...
    area.innerHTML = "<h2 style='color:white; margin-top:100px;'>CARREGANDO PRÉVIA...</h2>";
...
function renderCarometro(data) {
    ...
    const bg = (turno === 'manha') ? 'FUNDOMANHA.jpg' : 'FUNDOTARDE.jpg'; [cite: 17]
...
function renderCrachas(data) {
    const area = document.getElementById('pdf-area');
    const turno = (document.querySelector('input[name="turno"]:checked').value === 'manha') ? 'MANHÃ' : 'TARDE'; [cite: 21]
    const turma = document.getElementById('input-turma').value.toUpperCase();
    area.innerHTML = "";
    for (let i = 0; i < data.length; i += 8) { [cite: 22]
        const page = document.createElement('div');
        page.className = 'page-a4';
        data.slice(i, i + 8).forEach(item => {
            page.innerHTML += `
                <div style="width:92mm; height:60mm; border:1pt solid #ccc; display:flex; flex-direction:column; background:white; position:relative; overflow:hidden;">
                    <div style="height:18mm; display:flex; align-items:center; padding:5px; border-bottom:1px solid #ddd;">
                        ...
                        <div style="text-align:center; flex:1;">
                            <div style="font-size:7pt; font-weight:bold;">ESCOLA MUNICIPAL DOM MANUEL D’ELBOUX</div>
                        </div>
                    </div>
...
function renderEtiquetas(data) {
    ...
    const cor = (turno === 'manha') ? '#4A5D23' : '#003399';
    area.innerHTML = "";
    for (let i = 0; i < data.length; i += 8) { [cite: 30]
        const page = document.createElement('div');
        page.className = 'page-a4';
        data.slice(i, i + 8).forEach(item => {
            page.innerHTML += `
                <div style="width:90mm; height:63mm; border:0.5pt solid black; display:flex; flex-direction:column; background:white;">
                    <div style="height:17.5mm; border-bottom:2.5pt dotted ${cor}; display:flex; align-items:center; padding:5px;">
                        ...
                        <span style="font-size:8pt; font-weight:bold; color:black; flex:1; text-align:center;">DOM MANUEL DA SILVEIRA D’ELBOUX</span>
                    </div>
...
function doPPT() {
    ...
    document.querySelectorAll('.page-widescreen').forEach(p => {
        ...
        slide.addImage({ data:img, x:4.6, y:0.5, w:4.1, h:5.3 }); [cite: 40]
        slide.addText(nome, { x:0, y:6.2, w:'100%', align:'center', fontSize:42, bold:true, color:'000000' });
    });
    pptx.writeFile({ fileName: 'Carometro_DomManuel.pptx' }); [cite: 41]
}
