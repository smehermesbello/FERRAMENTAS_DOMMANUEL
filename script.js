...
function setupBtns(types) {
    ...
    if (types.includes('pdf')) {
        div.innerHTML += `<button id="btn-pdf" onclick="doPDF()" class="btn-liquid-small" style="background:rgba(39, 174, 96, 0.5);">📄 SALVAR PDF</button>`;
    }
    if (types.includes('ppt')) {
        div.innerHTML += `<button onclick="doPPT()" class="btn-liquid-small" style="background:rgba(230, 126, 34, 0.5);">📊 SALVAR PPTX</button>`;
    }
}

// Funções de Renderização (Mantidas conforme lógica original)
...
async function doPDF() {
    ...
    html2pdf().set(opt).from(element).save().then(() => btn.innerText = "📄 SALVAR PDF");
}
